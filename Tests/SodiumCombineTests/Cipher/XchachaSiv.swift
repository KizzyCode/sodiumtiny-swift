...
final class CipherXchachaSivTests: XCTestCase {
    /// A codable message
    ...
    func testXchachaSiv() throws {
        ...
        for _ in 0...16_384 {
            ...
            let message = Message(), index = Index(), zeroNonce = Bool.random(),
                cipher = try CipherXchachaSiv(key: key, zeroNonce: zeroNonce, encoder: encoder, decoder: decoder)
            ...
            // Validate the zero-nonce flag if set
            if zeroNonce {
                let ciphertext = try decoder.decode(Ciphertext.self, from: ciphertext)
                XCTAssert(ciphertext.iv.isEmpty)
            }
        }
    }
    ...
    func testXchachaSivError() throws {
        ...
        let encoder = JSONEncoder(), decoder = JSONDecoder(),
            key: Data = try Random().generate(count: 32),
            cipher = try CipherXchachaSiv(key: key, encoder: encoder, decoder: decoder)
